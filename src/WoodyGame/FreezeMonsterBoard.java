...
import WoodyGame.sprite.Freeze;
import WoodyGame.sprite.Gosma;
import WoodyGame.sprite.Monstro;
import WoodyGame.sprite.Woody;
...
public class FreezeMonsterBoard extends AbstractBoard {
    ...
    protected void createBadSprites() { // create monsters
        for (int i = 1; i <= 9; i++) {
            Monstro monstro = new Monstro(Commons.MONSTER_INIT_X + 18 * i, Commons.MONSTER_INIT_Y + 30 * i, i);
        ...
        if (key == KeyEvent.VK_SPACE) {
            ...
            if (inGame) {
                ...
                if (!shot.isVisible()) {
                    ...
                    shot = new Freeze(x, y, player.lastDirection);
                }
            }
        }
    }
            ...
            for (BadSprite monster : badSprites) {
                if (!monster.isFreezing() && shot.isVisible()) {
                    if (checkSpriteCollision(shot, monster)) {
                        Monstro monsterGetImage = (Monstro) monster;
                        String frozenImage = monsterGetImage.freezeImg;
    ...
    private void updateOtherSprites() {
        ...
        for (BadSprite monster : badSprites) {
            ...
            Gosma gosma = ((Monstro) monster).getBomb();
    ...
    private boolean leakedFromScreen(Sprite a1) {
        return a1.getY() < (-a1.getImageHeight()) || a1.getY() > d.getHeight()
                || a1.getX() < (-a1.getImageWidth()) || a1.getX() > d.getWidth();
    }
